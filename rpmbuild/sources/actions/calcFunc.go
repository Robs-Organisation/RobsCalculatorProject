...
func inputUser() string {
	var aufgabe string
	fmt.Println("Please insert your task: ")
	fmt.Scanf("%s", &aufgabe)
	return aufgabe
}

func aksForReRun() bool {

	CorrectInput := false
	fmt.Println("Do you want to use the calculator again? (y/n)")
	var input string
	var BoolInputUser bool

	for CorrectInput != true {
		_, Err := fmt.Scanf("%s", &input)
		if Err != nil {
			fmt.Println("Error bei der Eingabe!")
		}
		if input == "y" {
			CorrectInput = true
			BoolInputUser = true
			...
			fmt.Println("Error with your input!\nDo you want to use the calculator again? (y/n)")
		}
	}
	return BoolInputUser
}
